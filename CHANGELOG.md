...
앱 내장 가이드(`/guide`) 갱신 — 2026-08-04 VDI 재테스트 결과 반영.

### Added
- "일상 사용" 절에 다중 PC 동시 제어 안전장치(v1.6.2) 설명 신설 — 재부팅/직접 다이얼/회의
  참가가 통화 중이거나 상태 확인 불가 시 차단되는 동작과 안내 문구, 음소거/통화종료는
  영향받지 않는다는 점을 설명. "문제 해결" 절에 관련 FAQ 추가.
- "시작하기" 절에 로컬 포트(8765~8769) 자동 폴백 안내 추가(v1.5.1에서 구현됐으나 가이드에
  누락돼 있었음).
- "장비가 계속 오류/오프라인으로 보인다" 항목에 일시적 네트워크 단절 후 자동 재연결 안내
  추가.
- 버전 표기("BridgeX vX.Y.Z · coredxi 제작")를 설정 화면 안에서 사이드바 맨 아래(가이드
  링크 밑)로 이동 — 모든 화면에서 항상 보이도록 함(index/logs/settings/guide.html 전체
  반영).

### Fixed
- "재부팅" 설명이 "회의실 사용 중 누르면 즉시 통화가 끊긴다"고 돼 있었으나, v1.6.2 안전
  장치로 실제로는 차단되어 끊기지 않음 — 사실과 다르던 설명 수정.
...
- **안전 문제**: 여러 PC가 같은 장비를 각자 독립적으로 폴링·제어하는 상황에서, 한 PC가
  ...
  장비의 실제 통화 상태를 fresh하게 재확인하도록 수정, 통화 중이면 409로 차단. 장비 상태를
  확인할 수 없는 경우(오프라인/응답없음)도 동일하게 fail-closed로 409 차단
