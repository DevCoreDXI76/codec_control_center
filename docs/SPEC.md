...
**(2026-08-04 추가) 확장자 위장으로 메일 차단을 우회하지 말 것 — 사내 DRM이 자동으로 파일을
영구 손상시킨다.** 사내 메일의 exe 첨부 차단을 피하려고 확장자를 `.txt`로 바꿔 전송한 사례에서,
사내 문서보안(DRM) 솔루션이 첨부파일을 문서로 오인해 자동 암호화를 걸었다. 그 뒤 확장자를 다시
`.exe`로 되돌려도 파일 내용 자체가 이미 DRM으로 감싸진 상태라 더 이상 유효한 실행 파일이 아니며
복구 불가능하다(원본을 다시 받아야 함). 증상은 "파일이 손상됨" 안내가 아니라 Windows의 일반
호환성 오류("현재 PC에서는 이 앱을 실행할 수 없습니다 / PC 버전을 찾으려면 소프트웨어 게시자에게
문의하세요")로만 나타나 원인 파악이 어려웠다 — 아키텍처/OS 버전/S 모드를 모두 배제하고 나서야
확인됨. 위 문단의 "우회 시도 금지" 원칙에 해당하는 구체적 실패 사례다.

이후 사내 메일 대신 개인 웹메일(네이버 메일)로 zip 압축해 전송하는 방식으로 우회함 — Naver는
`.exe` 첨부는 막지만 `.zip`은 통과시키고, 사내 메일의 DRM 자동 태깅 경로를 타지 않아 손상 없이
전달됨. 다만 이는 사내 승인 경로가 아닌 개인 외부 메일로 사내 소프트웨어를 전송하는 것이라
보안/정책상 별도 검토가 필요할 수 있다 — 위 "사내 승인된 파일 전송 경로"가 여전히 원칙이며,
이 방법은 실제 사용된 임시 회피책으로만 기록해둔다.
